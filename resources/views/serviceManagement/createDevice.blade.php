...
<form method="POST" action="{{ route('serviceManagement.device.store') }}" class="mt-4 text-center">
    @csrf
    <div class="row">
    ...
    <div class="row mt-3">
        ...
        <div class="col-sm-10">
            <input type="text" name="uid" class="form-control" value="{{ old('uid') }}" required>
        </div>
    </div>
    <div class="row mt-3">
        <label class="col-sm-2 col-form-label">擺放地點</label>
        <div class="col-sm-10">
            <input type="text" name="storage_location" class="form-control" value="{{ old('storage_location') }}" required>
        </div>
    </div>
    <div class="row mt-3">
